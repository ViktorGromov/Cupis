...
$GLOBALS['drift_thinkup_prefix'] = 'drift_';
...
function drift_thinkup_customizer_setup() {
...
add_action( 'customize_register', 'drift_thinkup_customizer_setup' );

// Enqueue customizer scripts / styles
function drift_thinkup_customizer_adminscripts() {

	global $drift_thinkup_theme_version;

	// Add theme stylesheets.
	wp_enqueue_style( 'drift-thinkup-customizer', get_template_directory_uri() . '/admin/main/assets/css/thinkup-customizer.css', '', $drift_thinkup_theme_version );
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/lib/extentions/font-awesome/css/font-awesome.min.css', '', '4.6.3' );

	// Add theme scripts.
	wp_enqueue_script( 'drift-thinkup-customizer', get_template_directory_uri() . '/admin/main/assets/js/thinkup-customizer.js', array( 'jquery' ), $drift_thinkup_theme_version, true );

}
add_action( 'customize_controls_enqueue_scripts', 'drift_thinkup_customizer_adminscripts' );

// Assign custom function to fetch variable values
if ( !function_exists( 'drift_thinkup_var' ) ) {
	function drift_thinkup_var( $name, $key = false ) {
