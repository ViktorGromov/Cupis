 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'drift_thinkup_toolbox_about_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class drift_thinkup_toolbox_about_page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The drift_thinkup_toolbox_about_page class version.
		 */
		...
		/**
		 * The single instance of drift_thinkup_toolbox_about_page
		 *
		 * @var drift_thinkup_toolbox_about_page $instance The  drift_thinkup_toolbox_about_page instance.
		 */
		private static $instance;

		/**
		 * The Main drift_thinkup_toolbox_about_page instance.
		 *
		 * We make sure that only one instance of drift_thinkup_toolbox_about_page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof drift_thinkup_toolbox_about_page ) ) {
				self::$instance = new drift_thinkup_toolbox_about_page;
		...
		public function welcome_admin_notice() {
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'drift_thinkup_notice_welcome' ) ) {

					echo '<div class="thinkup-toolbox-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('drift-thinkup-hide-notice', 'welcome')), 'drift_thinkup_hide_notices_nonce', '_drift_thinkup_notice_nonce')) . '" style="z-index: 0;padding: 10px;text-decoration: none;" >';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'drift'). '</span>';
					echo '</a>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			if ( 'themes.php' === $pagenow || 'appearance_page_thinkup-welcome' == $hook_suffix ) {
				wp_enqueue_style( 'drift-thinkup-about-page-css', get_template_directory_uri() . '/admin/main-toolbox/assets/css/toolbox-backend.css' );
			}

			// enqueue js files
			if ( 'appearance_page_thinkup-welcome' == $hook_suffix ) {
				wp_enqueue_script( 'drift-thinkup-about-page-js', ( get_template_directory_uri() . '/admin/main-toolbox/assets/js/toolbox-backend.js' ), array( 'jquery' ), '', 'true' );
			}
		...
		public function hide_notice() {
			if (isset($_GET['drift-thinkup-hide-notice']) && isset($_GET['_drift_thinkup_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_drift_thinkup_notice_nonce'], 'drift_thinkup_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'drift'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'drift'));
				}
				$hide_notice = sanitize_text_field($_GET['drift-thinkup-hide-notice']);
				update_option('drift_thinkup_notice_' . $hide_notice, 1);
			}
		}
